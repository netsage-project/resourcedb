...
# This version will recalculate bits per second and packets per second using duration. (these were not adjusted for sampling corrections done in logstash)
...
my $index_names = "om-netsage-cenic-*";   # first part of names
...
########
#if ($index =~ /2020\./) { print ("SKIPPING $index\n"); next; }
                ...
                "filter" => [
                 {
                    "bool" => {
                        "should" => [
                            { "term" => { "meta.sensor_id" => "pw-sttl1-1.pacificwave.net" } },
                            { "term" => { "meta.sensor_id" => "wrn-denv-sw-1.cenic.net" } },
                            { "term" => { "meta.sensor_id" => "pw-snvl2-1.cenic.net" } },
                            { "term" => { "meta.sensor_id" => "pw-losa2-1.cenic.net" } }
                        ],
                        "minimum_should_match" => 1
                    }
                 },
                 {
                 "term" => { "stitched_flows" => "1" }
                 },
                 {
                 "range" => {
                    '@ingest_time' => {
                        "lt" => "2021-05-03 17:30:00",
                        "gt" => "2020-12-01 01:00:00",
