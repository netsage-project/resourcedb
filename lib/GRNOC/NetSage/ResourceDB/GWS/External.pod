...
GRNOC::NetSage::ResourceDB::GWS::User - GlobalNOC NetSage::ResourceDB Web Services Object for User core data

=head1 SYNOPSIS

  use GRNOC::NetSage::ResourceDB::GWS::User;

  my $websvc = GRNOC::NetSage::ResourceDB::GWS::User->new( config_file => '/etc/grnoc/netsage/resourcedb/config.xml' );
...
service requests for User related data (read-write requests).  Typically all you need to do is call the
...
also create a B<GRNOC::NetSage::ResourceDB::DataService::User> object and store it internally when
